...
agents-lint:
	...
	python scripts/check_weak_hashing.py $(shell find src test scripts -name '*.ts' -o -name '*.py')
	python scripts/check_dockerfile_root.py Dockerfile docker-compose.yml
	python scripts/check_secrets_heuristic.py $(shell find src test scripts -name '*.ts' -o -name '*.py') .github/workflows/*.yml .pre-commit-config.yaml Makefile Dockerfile docker-compose.yml
